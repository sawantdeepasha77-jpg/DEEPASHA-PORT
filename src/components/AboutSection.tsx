...
import { Sparkles, Activity, Compass, Sun, ArrowUpRight, HelpCircle } from 'lucide-react';
...
export const AboutSection: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className="pointer-events-none absolute right-0 top-1/4 h-96 w-96 rounded-full bg-purple-900/10 blur-[130px]"
        aria-hidden="true"
      />
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <h2
          ...
          className="text-section-title font-display font-bold text-white max-w-4xl"
        >
        ...
        <div className="mt-10 grid grid-cols-1 gap-10 md:grid-cols-12">
          <div className="md:col-span-8 space-y-6">
            ...
            <div className="pt-4 flex flex-wrap items-center gap-3">
              <span className="rounded-full border border-purple-500/30 bg-purple-950/40 px-3.5 py-1 text-xs font-mono text-purple-200">
                Curious by Default
              </span>
              <span className="rounded-full border border-purple-500/30 bg-purple-950/40 px-3.5 py-1 text-xs font-mono text-purple-200">
                Content &amp; Culture
          ...
          <div className="md:col-span-4 flex flex-col justify-between rounded-2xl border border-white/10 bg-[#120D1D]/60 p-6 backdrop-blur-sm">
            <div>
              ...
              {INTERESTS_DATA.map((item, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative flex w-full items-center justify-between rounded-xl px-5 py-4 text-left transition-all duration-300 interest-pill ${
                      isSelected
                        ? 'bg-[#1E1133] border border-purple-500/50 shadow-[0_0_25px_rgba(147,51,234,0.15)]'
                        : 'border border-transparent bg-white/[0.02] hover:bg-white/[0.05] hover:border-white/10'
                    }`}
                  >
                    <div className="flex items-center gap-4">
                      <span className="font-mono text-xs font-semibold text-purple-400/70">
                        0{idx + 1}
                      </span>
                      <span
                    ...
                    <div className="flex items-center gap-3">
                      {item.note && (
                        <span className="hidden sm:inline-block rounded-full bg-purple-950/70 border border-purple-400/30 px-2.5 py-0.5 text-[10px] font-mono text-purple-300">
                          {item.note}
                        </span>
                      )}
                      <ArrowUpRight
              ...
              <AnimatePresence mode="wait">
                {activeInterest && (
                  <motion.div
                    ...
                    className="h-full flex flex-col justify-between rounded-2xl border border-purple-500/30 bg-gradient-to-b from-[#170E28] to-[#0D0817] p-7 backdrop-blur-md shadow-[0_0_30px_rgba(147,51,234,0.12)]"
                  >
                    <div>
                      <div className="flex items-center justify-between">
                        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-purple-900/40 border border-purple-500/30">
                      ...
                      {activeInterest.note && (
                        <div className="mt-3 inline-flex items-center gap-1.5 rounded-md bg-purple-950/90 border border-purple-400/40 px-3 py-1 text-xs text-purple-200">
                    ...
                    <div className="mt-6 pt-5 border-t border-white/10">
                      <div className="flex flex-wrap gap-2">
                        {activeInterest.tags.map((tag) => (
                          <span
                            key={tag}
                            className="rounded-full bg-white/5 border border-white/10 px-2.5 py-1 text-[11px] font-mono text-zinc-300"
                          >
                            #{tag}
                          </span>
                        ))}
